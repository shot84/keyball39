...
// ---- ここから修正：名前衝突を避けるためQMK標準APIをラップ ---- //
static inline void hsv_to_rgb_bytes(uint8_t h, uint8_t s, uint8_t v,
                                    uint8_t *r, uint8_t *g, uint8_t *b) {
    HSV hsv = (HSV){ .h = h, .s = s, .v = v };
    RGB rgb = hsv_to_rgb(hsv); // QMK標準API（quantum/color.h）
...
report_mouse_t pointing_device_task_user(report_mouse_t mouse_report) {
    // 右手の動作が左マスターに送られてくる（SPLIT_POINTING_ENABLE 前提）
    uint16_t now = timer_read();
    // マウス移動

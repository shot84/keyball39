/*
 * Optimized keymap.c for size on AVR (no float, no sinf)
 */
...
enum {
    ...
    L_MOUSE= 3    // Layer3: マウス/スクロールムーブ（スクロール直後はスクロール表示）
};
...
static cursor_t g_cursor = {112, 32};   // 224x64座標系の中央
static uint16_t g_last_scroll_ms = 0;
static int8_t   g_scroll_band    = 0;   // 0..11 を想定（横帯インデックス）

extern led_config_t g_led_config;       // 既存宣言（constは付けない）

// ---- HSV→RGB（QMK標準APIのラッパ：命名衝突回避）----
...
// (1) アイスウェーブ：白基調＋青帯（整数版）
static void render_icewave(void) {
    uint16_t t = timer_read();
    uint16_t t8 = (t >> 2);   // 波の進行速度
    for (uint8_t i = 0; i < RGB_MATRIX_LED_COUNT; i++) {
        ...
        if (g_rainbow_mode) {
            uint8_t h; apply_palette_rainbow(t + (uint16_t)lx * 3, 160, &h);
...
static void render_scrollmove(void) {
    int16_t cx = (int16_t)((g_scroll_band * 224) / 12); // 12帯
    g_cursor.x = cx; g_cursor.y = 32;
...
// 右手のポインティング → 左マスターで受信（SPLIT_POINTING_ENABLE 前提）
report_mouse_t pointing_device_task_user(report_mouse_t m) {
    if (m.x || m.y) {
        g_cursor.x += (int16_t)m.x * 3;
        g_cursor.y -= (int16_t)m.y * 2;
        if (g_cursor.x < 0)        g_cursor.x += 224;
        else if (g_cursor.x >= 224) g_cursor.x -= 224;
        if (g_cursor.y < 0)        g_cursor.y = 0;
        else if (g_cursor.y > 64)  g_cursor.y = 64;
    }
    if (m.v) {
        g_scroll_band += (m.v > 0) ? 1 : -1;
        if (g_scroll_band < 0)  g_scroll_band = 11;
        if (g_scroll_band > 11) g_scroll_band = 0;
        g_last_scroll_ms = timer_read();
    }
    return m;
}

// カスタム描画：Layer0は純正、他レイヤを上書き
bool rgb_matrix_indicators_user(void) {
    uint8_t top = get_highest_layer(layer_state);
    if (top == L_BASE) return true; // SOLID_REACTIVE_MULTIWIDE を尊重
...
// ---------------- ここから：あなたの元 keymaps/g_led_config ----------------
// （元ファイルをそのまま保持）
